"""
AWS Glue job: pull ad-level performance data broken down by age bucket from
the Pinterest Ads API (v5) and upsert it into an Iceberg table in S3.
...
AGE_BUCKET breakdown instead of LOCATION. Produces a different grain of
table: one row per (ad, date, age_bucket) instead of one row per (ad, date).
...
this table is already scoped to one age bucket, so SUM(spend) for an
...
pinterest_ads_gender_to_iceberg_glue_job.py's docstring for the sibling job,
...
Glue job parameters expected (set as job arguments):
  ...
  --ICEBERG_TABLE             target table name, e.g. "pinterest_ad_age_performance"
...
Design notes specific to age breakdown:
- AGE_BUCKET is one of the 14 valid `targeting_types` values on
  ...
  on 2026-08-19. Its example targeting_value is "45-49" -- already a
  human-readable string, so unlike DMA (bare Nielsen codes needing a
  separate reference table), no lookup table is needed here. Note: the spec
  doesn't enumerate the complete set of age bucket values anywhere (only
  that one example), so don't assume a fixed list without checking a real
  response.
- Deliberately does NOT request AGE_BUCKET_AND_GENDER (the true age x gender
  cross-tab). Pinterest's spec flags it as "BETA and not yet available to
  all users" -- add it as a separate job once confirmed enabled on your
  accounts, rather than build against an unconfirmed BETA field.
...
logger = logging.getLogger("pinterest_ads_age_to_iceberg")
...
# deliberately kept identical so summing this table's metrics across age
# bucket for a given (ad, date) is comparable to that table's row for the
# same (ad, date) -- modulo any traffic Pinterest doesn't attribute to an
# age bucket. Verified against the `ReportingColumnSync` enum in Pinterest's
# published v5 OpenAPI spec on 2026-08-19.
ANALYTICS_COLUMNS = [
    "AD_ID",
    "AD_GROUP_ID",
    "CAMPAIGN_ID",
...
SCHEMA = StructType([
    ...
    StructField("age_bucket", StringType(), False),  # e.g. "45-49"
...
KEY_COLUMNS = ["ad_account_id", "ad_id", "age_bucket", "stat_date"]
...
def to_row(ad_account_id: str, age_bucket: str, stat_date: str, metrics: dict,
...
def main():
    ...
    logger.info("Pulling Pinterest ad age analytics for %s..%s across %d account(s)",
    ...
    for ad_account_id in ad_account_ids:
        ...
        for batch in chunked(entity_ids, ID_BATCH_SIZE):
            ...
            for item in breakdown_rows:
                age_bucket = item.get("targeting_value")
                metrics = item.get("metrics", {})
                stat_date = metrics.get("DATE", start_date)
                all_rows.append(to_row(ad_account_id, age_bucket, stat_date, metrics, ingested_at))

    logger.info("Fetched %d ad-age-day rows across %d ad account(s)", len(all_rows), len(ad_account_ids))
    ...
    upsert(spark, df, full_table_name, ICEBERG_COLUMNS, KEY_COLUMNS,
           PARTITION_EXPR, temp_view_name="pinterest_ads_age_source")
