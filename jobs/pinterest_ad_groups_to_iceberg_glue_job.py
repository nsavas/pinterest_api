...
Glue job parameters expected (set as job arguments):
  ...
  --ICEBERG_TABLE             target table name, e.g. "pinterest_ad_group_performance"
...
Design notes specific to the ad group level:
- Like campaigns/analytics, ad_groups/analytics requires ad_group_ids on
  every call (there's no "omit the filter to get everything" mode), so we
  always list ad group IDs first and batch analytics requests in chunks of
  250 (Pinterest's documented max ad_group_ids per call).
"""
...
logger = logging.getLogger("pinterest_ad_groups_to_iceberg")

ENTITY_PATH = "ad_groups"
ID_PARAM_NAME = "ad_group_ids"
ID_BATCH_SIZE = 250  # Pinterest's documented max ad_group_ids per analytics call
...
SCHEMA = StructType([
    ...
    StructField("ad_group_status", StringType(), True),
...
KEY_COLUMNS = ["ad_account_id", "ad_group_id", "stat_date"]
...
def to_row(ad_account_id: str, stat_date: str, record: dict, ingested_at: datetime) -> tuple:
    ...
    return (
        ...
        record.get("AD_GROUP_NAME"),
        record.get("AD_GROUP_ENTITY_STATUS"),
        stat_date,
...
def main():
    ...
    logger.info("Pulling Pinterest ad group analytics for %s..%s across %d account(s)",
    ...
    for ad_account_id in ad_account_ids:
        ...
        if not entity_ids:
            logger.info("Ad account %s has no ad groups, skipping", ad_account_id)
            continue
    ...
    logger.info("Fetched %d ad-group-day rows across %d ad account(s)", len(all_rows), len(ad_account_ids))
    ...
    upsert(spark, df, full_table_name, ICEBERG_COLUMNS, KEY_COLUMNS,
           PARTITION_EXPR, temp_view_name="pinterest_ad_groups_source")
